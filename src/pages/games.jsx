...
function Games() {
  ...
  var [gamesSearchTerm, setGamesSearchTerm] = React.useState("");

  function searchGamesType(e) {
    setGamesSearchTerm(e.target.value.toLowerCase());
  }
  ...
  const gitemssearched = gitems.filter((item) => {
    if (!gamesSearchTerm) {
      ...
      return item.name.toLowerCase().includes(gamesSearchTerm);
    }
  });
  return (
    <>
